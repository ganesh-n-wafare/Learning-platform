...
const buttonBase = 'inline-flex items-center justify-center rounded-full font-semibold transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600 focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-60';
...
function NotificationCenter() {
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, title: 'React lesson is ready', detail: 'Continue with Effects and data fetching.', unread: true },
    { id: 2, title: 'Weekly goal almost complete', detail: 'You are one lesson away from your goal.', unread: true },
    { id: 3, title: 'New recommendation', detail: 'Try Docker for Beginners next.', unread: false },
  ]);
    ...
    <div className="relative">
      ...
      {isOpen && <section ref={panelRef} id="notification-panel" aria-label="Notifications" className="absolute right-0 top-full z-30 mt-3 w-[min(22rem,calc(100vw-2rem))] overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
        <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
          ...
          <button type="button" onClick={markAllAsRead} className="text-sm font-semibold text-indigo-600 hover:text-indigo-700 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-600 focus-visible:ring-offset-2">Mark all read</button>
        </div>
        <ul className="max-h-80 divide-y divide-slate-100 overflow-y-auto">
          {notifications.map((notification) => <li key={notification.id} className={`px-4 py-3 ${notification.unread ? 'bg-indigo-50/60' : 'bg-white'}`}><p className="text-sm font-semibold text-slate-800">{notification.title}</p><p className="mt-1 text-sm leading-5 text-slate-600">{notification.detail}</p></li>)}
        </ul>
      </section>}
    </div>
  );
